      ...
      <div style="padding:0 15px 100px 15px;">
        ...
        >
          ...
          <el-table-column label="状态" align="center" prop="useStatus">
            <template slot-scope="scope">
              <span v-if="scope.row.useStatus == '一级核准'||scope.row.useStatus == '一级否决'">待审核</span>
          ...
          <el-table-column property="set" label="操作" align="center" width="150" fixed="right">
            <template slot-scope="scope">
              <el-button
                ...
                v-if="scope.row.useStatus == '一级核准'||scope.row.useStatus == '一级否决'"
              >审核</el-button>
...
import { secondGetUseLend, secondCheck } from "@/api/borrow";
export default {
  data() {
    return {
      pickerOptions: {
